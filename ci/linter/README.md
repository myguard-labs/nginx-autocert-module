...
`.github/workflows/ci-deep.yml`. `install-linters.sh` is the canonical pin;
`grep -rn 'semgrep==' ci/ .github/` lists every site that must move together.
...
- `git config core.hooksPath .githooks` — this directory: also covers
...
- `pipx install pre-commit && pre-commit install` — the framework: also runs
  ...
  staged patch, but not `lint-nginx.sh` or `lint-yaml.sh`'s yamllint/zizmor
  passes. Then leave `core.hooksPath` unset.

Running both means running flawfinder, semgrep and cppcheck twice per commit.
